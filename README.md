...
It has some similarities to other notebooks like Wolfram, R, Jupyter and Observable, but is primarily aimed at being used in the console, and tries to be "opinionated about being not opinionated" by emphasizing a pluggable architecture in which arbitrary languages and tools can be used for parsing cell input and displaying cell output. Here's a quick example:
...
* Data streams have first class support; unbuffered output or live data sources appear as it is available, not after the entire cell is finished executing.
...
* Displaying outputs of cells is also done with a plugout architectures, and new output/export options can be added as plugouts. Images, HTML, etc can be rendered in a browser. So despite being console focused, output in the browser is intended to be well-supported.
